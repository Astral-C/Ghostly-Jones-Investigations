...
void ghTextBoxSetColor(uint8_t r, uint8_t g, uint8_t b){
    textbox_manager.color = nk_rgb(r, g, b);
}
...
void ghTextBoxDraw(){
    ...
    if(textbox_manager.enabled){
        nk_style_push_style_item(textbox_manager.ctx, &textbox_manager.ctx->style.window.fixed_background, nk_style_item_color(nk_rgba(0,0,0,200)));
        ...
        nk_label_colored_wrap(textbox_manager.ctx, textbox_manager.text, textbox_manager.color);
